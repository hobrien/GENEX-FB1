...
Sleuth <- read_delim("../Results/SleuthPCW_RIN_annotated.txt", 
                     "\t", escape_double = FALSE, trim_ws = TRUE)

#select the transcript for each gene with the lowest qvalue
SleuthTopIsoform <- Sleuth %>% group_by(Id) %>% arrange(qval) %>% slice(1) %>% ungroup()
...
- This is a slightly tricky comparison because DESeq2 works on gene-level counts while Sleuth works with transcript-level TPMs from Kalliso
- Nevertheless, all DE genes should have one or more DE transcripts and DE transcripts should result in gene-level DE unless (A) the DE transcript is expressed at a low level relative to other isoforms or (B) isoform switching results in Male biased transcripts balancing out Female biased transcripts.
...
# There is one sig transcript that is not in the latest version of BioMart and therefore doesn't have an gene Id
SleuthSig <- (filter(SleuthTopIsoform, qval<.1 & ! is.na(Id)))
...
DESeq_vs_Sleuth <- filter(sigGenesDESeq, ageBin=='12-19') %>%
  dplyr::select(Id, SYMBOL, Chr, gene_type) %>%
  full_join(dplyr::select(SleuthSig, Id, SYMBOL, Chr, gene_type)) %>%
  left_join(dplyr::select(filter(DESeqfit, ageBin=='12-19'), Id, Sex, ChrType, DESeq_pvalue = pvalue, DESeq_padj = padj)) %>%
  left_join(dplyr::select(SleuthTopIsoform, Id, Sleuth_pvalue=pval, Sleuth_padj=qval)) %>%
...
ggplot(filter(DESeq_vs_Sleuth, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(Sleuth_padj)) , aes(x=DESeq_pvalue, y=Sleuth_pvalue, colour=Method)) + 
    ...
    #xlab('Sex Chromosomes Included') +
    #ylab('Sex Chromosomes Excluded') +
    ggtitle('Nominal p-values (excludes SDCBP2-AS1 Sleuth p<1e-40, DESeq p>.01)') +
    theme(legend.position = 'right')
...
ggplot(filter(DESeq_vs_Sleuth, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(Sleuth_padj)), aes(x=DESeq_padj, y=Sleuth_padj, colour=Method)) +
    ...
    scale_y_log10(limits=c(1e-10, 1)) + scale_x_log10(limits=c(1e-10, 1)) +
    #scale_y_log10() + scale_x_log10() +
    ggtitle('q-values (excludes SDCBP2-AS1 Sleuth q<1e-40, DESeq q>.1)') +
    theme(legend.position = 'right')
```

- One gene in particular (SDCBP2-AS1) is highly DE according to Sleuth and not at all according to DESeq
...
PlotTranscripts('SDCBP2-AS1', AllKallisto, Sleuth)
...
PlotTranscriptByAge <- function(transcriptID, AllKallisto, Sleuth){
    ...
    ggplot(counts, aes(x=PCW, y=TPM, colour=Sex)) +
      geom_jitter(height = 0, width=.25, alpha=.75) + 
      facet_wrap(~ target_id, ncol=10) +
      xlab('') +
      main_theme() +
...
PlotTranscriptByAge('ENST00000609745', AllKallisto, Sleuth)
