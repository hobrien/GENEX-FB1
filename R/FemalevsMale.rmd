...
```{r DESeq_vs_Sleuth}
#read in Sleuth results and merge with gene info (using BioMart)
...
Sleuth <- right_join(t2g, mutate(Sleuth, target_id=str_replace(target_id, '\\.[0-9]+', ''))) %>% mutate(Chr=paste0('chr', Chr))
...
AllKallisto <- read_delim("~/BTSync/FetalRNAseq/Github/GENEX-FB1/Counts/AllKallisto.txt", 
                          "\t", escape_double = FALSE, trim_ws = TRUE)

AllKallisto <- right_join(t2g, mutate(AllKallisto, target_id=str_replace(target_id, '\\.[0-9]+', '')))

PlotTranscripts <- function(geneID, AllKallisto, Sleuth){
    geneID = sub("(ENSG[0-9]+)\\.[0-9]+", '\\1', geneID)
    counts <- filter(AllKallisto, Id == geneID | SYMBOL == geneID) %>% 
      dplyr::select(-Id, -SYMBOL, -Chr, -gene_type) %>% 
      gather(Sample, TPM, -target_id) %>% 
    ...
    p <- ggplot(counts, aes(x=Sex, y=TPM, colour=factor(PCW))) +
      ...
      scale_colour_brewer(type = "qual", palette = 6) +
      ggtitle(paste0(filter(counts, !is.na(Id))$Id[1], ' (', filter(counts, !is.na(SYMBOL))$SYMBOL[1], ')'))
   p
}
PlotTranscripts('SDCBP2-AS1', AllKallisto, Sleuth)

PlotTranscriptByAge <- function(geneID, AllKallisto, Sleuth){
    geneID = sub("(ENSG[0-9]+)\\.[0-9]+", '\\1', geneID)
    counts <- filter(AllKallisto, target_id == geneID) %>% 
      dplyr::select(-Id, -SYMBOL, -Chr, -gene_type) %>% 
      gather(Sample, TPM, -target_id) %>% 
      left_join(LibraryInfo) %>%
      left_join(Sleuth) %>%
