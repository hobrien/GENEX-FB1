...
DESeqfit <- DESeqfit %>%  mutate(Sex = ifelse(log2FoldChange < 0, 'F', 'M'),
  ...
  left_join(dplyr::select(gene_info, Id, ChrType), by='Id')
...
sigGenes <- semi_join(sigGenesEdgeR, dplyr::select(sigGenesDESeq, analysis), by='analysis') %>% 
  mutate(Method='Both') %>%
  bind_rows(anti_join(sigGenesEdgeR, dplyr::select(sigGenesDESeq, analysis), by='analysis')) %>%
  bind_rows(anti_join(sigGenesDESeq, dplyr::select(sigGenesEdgeR, analysis), by='analysis')) %>%
...
Sleuth <- read_delim("../Results/SleuthGenePCW_RIN.txt", 
                     "\t", escape_double = FALSE, trim_ws = TRUE)
Sleuth <- dplyr::rename(Sleuth, Id=target_id)
Sleuth <- right_join(gene_info, Sleuth)

SleuthIsoform <- read_delim("../Results/SleuthPCW_RIN_annotated.txt", 
                     "\t", escape_double = FALSE, trim_ws = TRUE)

#select the transcript for each gene with the lowest qvalue
SleuthTopIsoform <- SleuthIsoform %>% group_by(Id) %>% arrange(qval) %>% dplyr::slice(1) %>% ungroup()

fittedBias <- read_tsv("../Shiny/GENEX-FB1/Data/fitted.txt") %>% left_join(dplyr::select(gene_info, Id, gene_type))
fittedBias_tr <- read_tsv("../Shiny/GENEX-FB1/Data/fitted_tr.txt")  %>% left_join(dplyr::select(gene_info, Id, gene_type))
fittedBias_topIsoform <- fittedBias_tr %>% group_by(Id) %>% arrange(padj) %>% dplyr::slice(1) %>% ungroup()
#left_join(fittedBias_topIsoform, dplyr::select(gene_info, Id, gene_type), by=c('GeneID'))
```
...
p1 <- count_matrix %>%
  ...
  ggplot(aes(x=PCW, y=n)) +
    ...
    xlab("Post Conception Weeks") +
    ylab("Ensembl genes (66023 total)") +
    theme(axis.text.x=element_text(size=8)) +
...
DESeq_vs_EdgeR <- group_by(sigGenes, analysis) %>%
  ...
  dplyr::select(analysis, DESeq_pvalue=pvalue, DESeq_padj=padj, ageBin, baseMean) %>%
  left_join(dplyr::select(EdgeRfit, analysis, EdgeR_pvalue=pvalue, EdgeR_padj=padj), by='analysis') %>%
  left_join(dplyr::select(DESeqCooks, analysis, maxCooks), by='analysis')
...
# Add Surrogate Variable Analysis

``` {r sva}
DESeqfitSVA <- read_delim("../Results/Sex_PCW_12_20_FDR_0.1_DESeq_sva3/tables/BG12_20.txt", "\t", escape_double = FALSE, trim_ws = TRUE, col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
  mutate(ageBin='12-19')

DESeqfitSVA_sig <- filter(DESeqfitSVA, padj<.1)

DESeq_vs_DESeqSVA <- filter(sigGenesDESeq, ageBin=='12-19') %>%
  dplyr::select(Id, SYMBOL, Chr, gene_type) %>%
  full_join(dplyr::select(DESeqfitSVA_sig, Id, SYMBOL, Chr, gene_type), by = c("Id", "SYMBOL", "Chr", "gene_type")) %>%
  left_join(dplyr::select(filter(DESeqfit, ageBin=='12-19'), Id, Sex, ChrType, DESeq_pvalue = pvalue, DESeq_padj = padj), by = "Id") %>%
  left_join(dplyr::select(DESeqfitSVA, Id, DESeqSVA_pvalue=pvalue, DESeqSVA_padj=padj), by = "Id") %>% 
  mutate(Method=ifelse(!is.na(DESeqSVA_padj) & DESeqSVA_padj < .1,
                       ifelse(!is.na(DESeq_padj) & DESeq_padj < .1, 
                              'Both Sig',
                              'SVA Sig'),
                       'DESeq Sig'),
          Tested=ifelse(is.na(DESeqSVA_padj),
                             'DESeq Tested',
                             ifelse(is.na(DESeq_padj), 
                                    'SVA Tested', 
                                    'Both Tested')
                        ),
         ChrType=ifelse(is.na(ChrType), 
                        ifelse(Chr %in% c('chrX', 'chrY'), Chr, 'autosomal'),
                        ChrType)
        ) %>% 
  group_by(Id) %>% slice(1) %>% ungroup() 

dplyr::group_by(DESeq_vs_DESeqSVA, Tested, ChrType, Method) %>% summarise(n=n()) %>% spread(Method, n)

ggplot(DESeq_vs_DESeqSVA, aes(x=Sex, fill=factor(ChrType, levels=c('autosomal', 'PAR', 'chrX', 'chrY')))) + geom_bar(position='stack') + 
    figure_theme() +
    facet_grid(Tested ~ Method) +
    #scale_y_continuous(breaks=seq(100,1000,100)) +
    scale_fill_manual(values=brewer.pal(6, "Set1")[c(1,6,4,2)]) +
    theme(strip.switch.pad.grid = unit(10, 'cm')) +
    ylab('Number sex-biased genes (FDR<0.1)') +
...
ggplot(filter(DESeq_vs_DESeqSVA, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(DESeqSVA_padj)) , aes(x=DESeq_pvalue, y=DESeqSVA_pvalue, colour=Method)) + 
    geom_point() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
    geom_vline(xintercept=.1, linetype=3)+
    figure_theme() +
    #scale_y_log10(limits=c(1e-12, 1)) + scale_x_log10(limits=c(1e-12, 1)) +
...
ggplot(filter(DESeq_vs_DESeqSVA, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(DESeqSVA_padj)), aes(x=DESeq_padj, y=DESeqSVA_padj, colour=Method)) +
    geom_point() +
    figure_theme() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
    ...
    #xlab('Sex Chromosomes Included') +
    #ylab('Sex Chromosomes Excluded') +
    #scale_y_log10(limits=c(1e-10, 1)) + scale_x_log10(limits=c(1e-10, 1)) +
    scale_y_log10() + scale_x_log10() +
    ggtitle('q-values') +
    theme(legend.position = 'right')
...
# Compare DESeq2 to Sleuth
- Correspondance of p-values is generally poor
- all DESeq genes with FDR < 0.001 (3 genes) are also sig by Sleuth
- of 14 Sleuth genes with FDR < 0.001, only 6 were tested by DESeq and only 3 were significant (including 6 with Sleuth FDR < 1e-7 where one was tested by DESeq and it was not significant)


- Both methods agree on Y genes.
- Sleuth fails to pull out most PAR genes and some other X genes
- Lists of autosomal genes are mostly orthogonal (3 in common out of 61)
- Many autosomal genes tested only by Sleuth and some Y genes tested only by DESeq

``` {r Sleuth_vs_DESeq}
SleuthSig <- (filter(Sleuth, qval<.1 & ! is.na(Id)))

DESeq_vs_Sleuth <- filter(sigGenesDESeq, ageBin=='12-19') %>%
  dplyr::select(Id, SYMBOL, Chr, ChrType, gene_type) %>%
  full_join(dplyr::select(SleuthSig, Id, SYMBOL, Chr, ChrType, gene_type)) %>%
  left_join(dplyr::select(filter(DESeqfit, ageBin=='12-19'), Id, Sex, ChrType, DESeq_pvalue = pvalue, DESeq_padj = padj)) %>%
  left_join(dplyr::select(Sleuth, Id, Sleuth_pvalue=pval, Sleuth_padj=qval)) %>%
  mutate(Method=ifelse(!is.na(Sleuth_padj) & Sleuth_padj < .1,
                       ifelse(!is.na(DESeq_padj) & DESeq_padj < .1, 
                              'Both Sig',
                              'Sleuth Sig'),
                       'DESeq Sig'),
          Tested=ifelse(is.na(Sleuth_padj),
                             'DESeq Tested',
                             ifelse(is.na(DESeq_padj), 
                                    'Sleuth Tested', 
                                    'Both Tested')
...
ggplot(DESeq_vs_Sleuth, aes(x=Sex, fill=factor(ChrType, levels=c('autosomal', 'PAR', 'chrX', 'chrY')))) + geom_bar(position='stack') + 
    figure_theme() +
    facet_grid(Tested ~ Method) +
    scale_fill_manual(values=brewer.pal(6, "Set1")[c(1,6,4,2)]) +
    theme(strip.switch.pad.grid = unit(10, 'cm')) +
...
ggplot(filter(DESeq_vs_Sleuth, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(Sleuth_padj)) , aes(x=DESeq_pvalue, y=Sleuth_pvalue, colour=Method)) + 
    geom_point() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
    geom_vline(xintercept=.1, linetype=3)+
...
ggplot(filter(DESeq_vs_Sleuth, ChrType=='autosomal' & ! is.na(DESeq_padj) & ! is.na(Sleuth_padj)), aes(x=DESeq_padj, y=Sleuth_padj, colour=Method)) +
    geom_point() +
    figure_theme() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
...
- All DE genes should have one or more DE transcripts and DE transcripts should result in gene-level DE unless (A) the DE transcript is expressed at a low level relative to other isoforms or (B) isoform switching results in Male biased transcripts balancing out Female biased transcripts
- 
...
TopIsoformSig <- (filter(fittedBias_topIsoform, padj<.1 & ! is.na(Id)))
SigGenes <- filter(fittedBias, padj<.1)
# Need to join on Id, SYMBOL, Chr and gene_type because otherwise if these are added from one of the data sets, the values are NA for any genes that weren't tested for that dataset.
# Unfortunately I've uesed different data sources for assigning SYMBOLs to the gene IDs. This means that some rows are duplicated. The group/slice/ungroup at the end takes care of this
gene_vs_isoform 
  
SigGenes %>%
  dplyr::select(Id, SYMBOL, Chr, gene_type) %>% View()
  full_join(dplyr::select(SleuthTopIsoformSig, Id, SYMBOL, Chr, gene_type)) %>% 
  left_join(dplyr::select(Sleuth, Id, ChrType, gene_pvalue = pval, gene_padj = qval)) %>%
  left_join(dplyr::select(SleuthTopIsoform, Id, isoform_pvalue=pval, isoform_padj=qval)) %>% 
  mutate(Method=ifelse(!is.na(gene_padj) & gene_padj < .1,
                       ifelse(!is.na(isoform_padj) & isoform_padj < .1, 
                              'Both Sig',
                              'Gene Sig'),
                       'Isoform Sig'),
          Tested=ifelse(is.na(gene_padj),
                             'Isoform Tested',
                             ifelse(is.na(isoform_padj), 
                                    'Isoform Tested', 
                                    'Both Tested')
                        ),
         ChrType=ifelse(is.na(ChrType), 
...
#filter(gene_vs_isoform_Sleuth, ChrType=='autosomal') 
ggplot(gene_vs_isoform_Sleuth, aes(x=Method, fill=factor(ChrType, levels=c('autosomal', 'PAR', 'chrX', 'chrY')))) + geom_bar(position='stack') + 
    figure_theme() +
    facet_grid(Tested ~ .) +
    #scale_y_continuous(breaks=seq(100,1000,100)) +
    scale_fill_manual(values=brewer.pal(6, "Set1")[c(1,6,4,2)]) +
...
ggplot(filter(gene_vs_isoform_Sleuth, ! is.na(isoform_padj) & ! is.na(gene_padj)) , aes(x=gene_pvalue, y=isoform_pvalue, colour=Method)) + 
    geom_point() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
    geom_vline(xintercept=.1, linetype=3)+
...
ggplot(filter(gene_vs_isoform_Sleuth, ! is.na(gene_padj) & ! is.na(isoform_padj)), aes(x=gene_padj, y=isoform_padj, colour=Method)) +
    geom_point() +
    figure_theme() +
    geom_abline(slope=1, intercept=0) +
    geom_hline(yintercept=.1, linetype=3)+
    ...
    theme(legend.position = 'right')
```
# plot genes with major differences between gene-level and transcript-level analyses

- SDCBP2-AS1 is highly DE according to the Sleuth isoform analysis and less sig according to the gene-level analysis
    - This antisense transcript has 4 isoforms that are expressed at a moderately high level in both sexes but one is expressed very highly in some intermediate-age females
- ANOS1 is highly significant according to isoform analysis and non-significant according to the gene-lvel analysis
    - this is because there is a DE transcript that is expressed at much lower levels than the main transcript
- AMY1A is similar, but there's only one isoform
- DDX3X and ARSD have a lot of isoforms, one of which is DE, but it doesn't look that convincing (FDRs around 1e-4)

- Not that if there are large differences in isoform lengths, count differences will be obscured because I'm plotting TPM which is length-normalised. Kallisto also produces estimated counts, which I could plot instead, but there are trade-offs with both approaches.

```{r}
AllKallisto <- read_delim("../Counts/AllKallisto.txt", 
                          "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  mutate(target_id=str_replace(target_id, '\\.[0-9]+', ''))

PlotTranscripts <- function(geneID, AllKallisto, Sleuth){
    geneID = sub("(ENSG[0-9]+)\\.[0-9]+", '\\1', geneID)
    counts <- filter(Sleuth, Id == geneID | SYMBOL == geneID) %>% 
      dplyr::select(target_id, pval, qval) %>%
      left_join(AllKallisto) %>% 
      gather(Sample, TPM, -target_id, -pval, -qval) %>% 
      left_join(LibraryInfo) %>% 
      mutate(target_id=paste0(target_id, '\nFDR=', signif(qval, digits = 3)),
             Sex=ifelse(Sex=='Male', 'M', ifelse(Sex=='Female', 'F', NA)))

    p <- ggplot(counts, aes(x=Sex, y=TPM, colour=Sex)) +
      geom_jitter(height = 0, width=.25, alpha=.75) + 
...
PlotTranscriptByAge <- function(transcriptID, AllKallisto, Sleuth){
    transcriptID = sub("(ENST[0-9]+)\\.[0-9]+", '\\1', transcriptID)
    counts <- filter(Sleuth, target_id == transcriptID) %>% 
      dplyr::select(target_id, pval, qval) %>%
      left_join(AllKallisto) %>% 
      gather(Sample, TPM, -target_id, -pval, -qval) %>% 
      left_join(LibraryInfo) %>% 
    ...
    p <- ggplot(counts, aes(x=PCW, y=TPM, colour=Sex)) +
      geom_jitter(height = 0, width=.25, alpha=.75) + 
      facet_wrap(~ target_id, ncol=10) +
      xlab('') +
      main_theme() +
      theme(legend.position='bottom') +
      scale_colour_brewer(type = "qual", palette = 6) +
      ggtitle(transcriptID)
   p
}

PlotTranscripts('SDCBP2-AS1', AllKallisto, SleuthIsoform)
PlotTranscriptByAge('ENST00000609745', AllKallisto, SleuthIsoform)

PlotTranscripts('ANOS1', AllKallisto, SleuthIsoform)
PlotTranscriptByAge('ENST00000481896', AllKallisto, SleuthIsoform)

PlotTranscripts('AMY1A', AllKallisto, SleuthIsoform)

PlotTranscripts('DDX3X', AllKallisto, SleuthIsoform)
PlotTranscriptByAge('ENST00000629496', AllKallisto, SleuthIsoform)

PlotTranscripts('ARSD', AllKallisto, SleuthIsoform)
PlotTranscriptByAge('ENST00000381154', AllKallisto, SleuthIsoform)
...
sigGenes %>% filter(ChrType == 'autosomal') %>% 
  group_by(gene_type) %>% 
  dplyr::summarise(autosomalDE=n()/sigAutosomesNum) %>% 
...
BG %>% 
  group_by(gene_type) %>%
  dplyr::summarise(background=n()/dim(BG)[1]) %>%
...
AutosomeExclusion <- filter(sigGenesDESeq, ChrType=='autosomal') %>%
  dplyr::select(analysis, XYpvalue=pvalue, XYpadj=padj, ageBin) %>%
  left_join(dplyr::select(AutosomesFit, analysis, Autosomes_pvalue=pvalue, Autosomes_padj=padj), by='analysis')
...
```{r pca}
vst <- read_tsv('../Results/Sex_PCW_12_20_FDR_0.1_DESeq_sva3/tables/counts_vst.txt')
metadata<-read_tsv('../Results/Sex_PCW_12_20_FDR_0.1_DESeq_sva3/tables/col_data.txt', col_types=cols(Sample='c'))

vst$var <- rowVars(as.matrix(dplyr::select(vst, -Id)))
pca <- arrange(vst, desc(var)) %>% .[1:500,] %>% dplyr::select(-Id, -var) %>% t() %>% prcomp()
prp <- pca$sdev^2 * 100 / sum(pca$sdev^2)
prp <- round(prp,2)

pca_df <- fortify(pca)
pca_df$Sample <- row.names(pca_df)
pca_df <- left_join(pca_df, metadata)


# axes 1 et 2
PlotPCA <- function(pca_df, prp, colour, shape="Sex") {

  p1 <- ggplot(pca_df, aes_string(x="PC1", y="PC2", colour=colour, shape=shape)) + 
    geom_point() +
	...
	# axes 3 et 4
  p2 <- ggplot(pca_df, aes_string(x="PC4", y="PC3", colour=colour, shape=shape)) + 
    geom_point() +
    stat_ellipse(aes(group = Sex)) +
  ...
  if (is.numeric(pca_df[[colour]])) {
    grid.arrange(p1+scale_colour_distiller(type="seq", palette=15),
                 p2+scale_colour_distiller(type="seq", palette=15), 
                 ncol=2)
  } else {
    grid.arrange(p1+scale_colour_brewer(type="qual", palette=6),
                 p2+scale_colour_brewer(type="qual", palette=6), 
                 ncol=2)
  }
}
PlotPCA(pca_df, prp, "Sex")
PlotPCA(pca_df, prp, "ReadLength")
PlotPCA(pca_df, prp, "RIN")
PlotPCA(pca_df, prp, "PCW")
PlotPCA(pca_df, prp, "V1")
PlotPCA(pca_df, prp, "V2")
PlotPCA(pca_df, prp, "V3")
...
idio<-plotIdeogram(hg19IdeogramCyto, "chrX", aspect.ratio = 1/20)
...
```
- A large number of genes symbols have multiple entries in the Ballaton table
- Almost all of these are classified as "no call"
- There are two genes labeled FAM156A that are both S and two genes labeled SPACA5, one of which is Mostly S and the other one of which is "no call"
- Since the classifications are almost always consistent, I could just remove the duplicates, but since most of them are "no call" which isn't very interesting anyway, I'm just going to filter all of these out for now.
- XIST is excluded

```{r}
XCI <- read_delim("~/BTSync/FetalRNAseq/Github/GENEX-FB1/Data/BallatonXCI.txt",
"\t", escape_double = FALSE, col_names = c("SYMBOL", "Status"),
trim_ws = TRUE)

XCI <- group_by(XCI, SYMBOL) %>% dplyr::summarise(n=n()) %>% filter(n==1) %>% left_join(XCI) %>% dplyr::select(-n)

left_join(XCI, fittedBias) %>%
  mutate(sig=ifelse(padj<.1, 1,0)) %>%
  filter(SYMBOL != 'XIST') %>%
  ggplot(aes(x=Status, y=log2FoldChange, colour=factor(sig))) +
...
left_join(XCI, fittedBias) %>%
  mutate(sig=ifelse(padj<.1, 1,0)) %>%
  filter(is.na(padj) & Status != 'No call')

filter(XCI, Status != 'No call') %>% dim()
```

# GTEx Analysis
- supp table 1 summarises previous findings by Cotton et al and Carrel & Willard
- supp table 3 lists 9 genes that have not previously been reported as escape but which have similar expression patterns as escape genes (see fig 2d in Tukiainen et al)
- supp table 13 lists a subset of the genes in supp table 1 (563/631), plus 120 additional genes that are listed as "Unknown"
- supp table 13 has info about ASE and SS expression, and marks 7 genes as NewCall (6 previously reported as inactive and one as unknown)
- this includes 4/5 variable ASE genes from fig 3g-k (-USP11)
- it also includes 4/7 variable genes from single-cell analysis in fig4e-k (-FTX/RPL36A/PHF8)
- FHL1 is shared between methods
- unclear why the other 4 genes are not considered new calls
- CHM is in both the list of novel escape genes and the list of novel variable genes for some reason. I guess I'll call it variable

- 583/631 genes in GTEx analysis can be matched to genes in Ballaton et al.
- 27/27 E and 21/25 mostly E genes classified as escape in GTEx (as are 17/17 PAR genes)
- 30/31 VE and 7/9 mostly VE genes classified as variable
- 273/286 S and 90/110 mostly S genes classified as inactive (11 as variable and 2 as escape)
...
group_by(XCIsummary, CombinedXCIstatus) %>% summarise(n=n())
GTExXCI <- read_delim("~/BTSync/FetalRNAseq/Github/GENEX-FB1/Data/GTExXCI.txt", "\t", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE)

dplyr::select(XCIsummary, GeneID, contains('XCIstatus')) %>% full_join(GTExXCI) %>% group_by(CombinedXCIstatus, ReportedStatus) %>% summarise(n=n())

filter(GTExXCI, NewCall=='x')

inner_join(XCI, XCIsummary, by=c('SYMBOL'='GeneName')) %>% dim()
inner_join(XCI, XCIsummary, by=c('SYMBOL'='GeneName')) %>% dplyr::select(Status, CombinedXCIstatus) %>% table()
```
# Overlap between DEGs and gene set members
- all overlapping genes in the gene-level analysis are also present in the transcript-level analysis
- does not include three genes from X-linked ID set which could not be associated with Ensembl IDs (HPRT, BRND3, DNDP)
- need to be cautious about interpreting overlap with X-linked ID genes since chrX genes are generally female biased. I guess the appropriate background is all expressed chrX genes?
```{r enrichment}
gene_sets <-read_delim("../Data/GeneSets.txt", 
                       "\t", escape_double = FALSE, 
                       trim_ws = TRUE)
colnames(gene_sets) <- c('SYMBOL', 'set', 'reference', 'Id')
disease2gene <- bind_rows(filter(gene_sets, set %in% c('SandersASD', 'PitonID'))[, c("set", "Id")],
                          dplyr::select(fittedBias, Id) %>% mutate(set='Background'))

FemaleDEG <- fittedBias %>% filter(padj<0.1 & log2FoldChange < 0) %>% 
  dplyr::select(Id, SYMBOL) %>%
  mutate(Direction='FemaleDEG', Analysis="Gene-Level")
MaleDEG <- fittedBias %>% filter(padj<0.1 & log2FoldChange > 0) %>%
  dplyr::select(Id, SYMBOL) %>%
  mutate(Direction='MaleDEG', Analysis="Gene-Level")
FemaleDEG_tr <- fittedBias_tr %>% filter(padj<0.1 & log2FoldChange < 0) %>% 
  dplyr::select(Id=GeneId, SYMBOL) %>%
  group_by(Id) %>%
  dplyr::slice(1) %>%
  mutate(Direction='FemaleDEG', Analysis="Transcript-Level")
MaleDEG_tr <- fittedBias_tr %>% filter(padj<0.1 & log2FoldChange > 0) %>%
  dplyr::select(Id=GeneId, SYMBOL) %>%
  group_by(Id) %>%
  dplyr::slice(1) %>%
  mutate(Direction='MaleDEG', Analysis="Transcript-Level")

bind_rows(FemaleDEG, MaleDEG, FemaleDEG_tr, MaleDEG_tr) %>%
  inner_join(dplyr::select(gene_sets, -SYMBOL), by='Id')
