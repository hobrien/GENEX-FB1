...
gene_info <- read_tsv("../Data/genes.txt") %>%
  ...
  dplyr::select(Id = gene_id, SYMBOL=gene_name, Chr=seqid, ChrType, gene_type)

par <- read_tsv("../Data/PARgenes.txt")
gene_info <- mutate(gene_info, ChrType=ifelse(SYMBOL %in% par$`Approved Symbol`, 'PAR', ChrType))
...
sigGenesDESeq <- read_delim("../Results/Sex_PCW_12_20_FDR_0.1_DESeq/tables/BG12_20.txt", "\t", escape_double = FALSE, trim_ws = TRUE, col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
  #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
  mutate(ageBin='12-19') %>%
  bind_rows(
    read_delim("../Results/Sex_12_FDR_0.1_DESeq/tables/BG12.txt", "\t", escape_double = FALSE, trim_ws = TRUE, col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
      #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
      mutate(ageBin='12')
  ) %>%
  bind_rows(
    read_delim("../Results/Sex_13_FDR_0.1_DESeq/tables/BG13.txt", "\t", escape_double = FALSE, trim_ws = TRUE, col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
      #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
      mutate(ageBin='13')
  ) %>%
  bind_rows(
    read_delim("../Results/Sex_14_FDR_0.1_DESeq/tables/BG14.txt", "\t", escape_double = FALSE, trim_ws = TRUE, col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
      #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
      mutate(ageBin='14')
  ) %>%
  bind_rows(
    read_delim("../Results/Sex_15_17_FDR_0.1_DESeq/tables/BG15_17.txt", "\t", escape_double = FALSE, trim_ws = TRUE,  col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
      #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
      mutate(ageBin='15-16')
  ) %>%
  bind_rows(
    read_delim("../Results/Sex_17_20_FDR_0.1_DESeq/tables/BG17_20.txt", "\t", escape_double = FALSE, trim_ws = TRUE,  col_types = cols(Id='c', SYMBOL='c', Chr='c', baseMean='d', 
Male='i',Female='i',FC='d', log2FoldChange='d', pvalue='d', padj='d')) %>%
      #dplyr::select(Id, log2FoldChange, pvalue, padj) %>% 
      mutate(ageBin='17-19')
  ) %>%
...
sigGenesEdgeR <- fittedBias %>% 
  bind_rows(fittedBiasAll) %>%
  filter(padj < .1 ) %>%
  mutate(Method='EdgeR', analysis=paste0(Id, ageBin))
...
sigGenes <- semi_join(sigGenesEdgeR, select(sigGenesDESeq, analysis), by='analysis') %>% 
  ...
  mutate(ChrType=ifelse(SYMBOL %in% par$`Approved Symbol`, 'PAR', ChrType))


ggplot(sigGenes, aes(x=Sex, fill=ChrType)) + geom_bar(position='stack') + 
  facet_grid(Method ~ ageBin) +
    figure_theme() +
    #scale_y_continuous(breaks=seq(100,1000,100)) +
    scale_fill_manual(values=brewer.pal(6, "Set1")[c(1,2,4,6)]) +
...
sigGenes <- sigGenes %>% left_join(select(gene_info, Id, gene_type), by='Id') 

ggplot(filter(sigGenes, Method != 'EdgeR'), aes(x=Sex, fill=gene_type)) + geom_bar(position='stack') + 
...
sigAutosomesNum <- dim(filter(sigGenes, ChrType == 'autosome'))[1]
sigGenes %>% filter(ChrType == 'autosome') %>% 
  group_by(gene_type) %>% 
  summarise(n=n()/sigAutosomesNum) %>% 
  arrange(desc(n))
...
select(gene_info, Id, gene_type) %>% 
  right_join(BG, by='Id') %>% 
  group_by(gene_type) %>%
  summarise(n=n()/dim(BG)[1]) %>%
  arrange(desc(n))
```
...
Fig3a <- PlotTimepoint('GABRA1', MalevsFemale_complete, fittedBias, LibraryInfo, '14', 'GABRA1')
Fig3b <- PlotTimepoint('GRIN2A', MalevsFemale_complete, fittedBias, LibraryInfo, '14', 'GRIN2A')
Fig3c <- PlotTimepoint('NES', MalevsFemale_complete, fittedBias, LibraryInfo, '14', 'NES')
Fig3d <- PlotTimepoint('EOMES', MalevsFemale_complete, fittedBias, LibraryInfo, '14', 'EOMES')
...
- there is some separation of new and old samples along PC1

```{r pca}
vst <- read_tsv('../Results/Sex_12_FDR_0.1_DESeq/tables/counts_vst.txt')
