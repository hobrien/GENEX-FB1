...
sigGenesDESeq <- read_delim("../Results/Sex_PCW_12_20_FDR_0.1_DESeq/tables/BG12_20.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
...
sigGenes <- semi_join(sigGenesEdgeR, select(sigGenesDESeq, analysis)) %>% 
  mutate(Method='Both') %>%
  bind_rows(anti_join(sigGenesEdgeR, select(sigGenesDESeq, analysis))) %>%
  bind_rows(anti_join(sigGenesDESeq, select(sigGenesEdgeR, analysis))) %>%
...
sigGenes <- sigGenes %>% left_join(select(gene_info, Id, gene_type)) 
