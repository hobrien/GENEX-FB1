...
- A lor of the genes that are DE with EdgeR were thrown out by DESeq because of high maxCooks scores
...
DESeqCooks <- read_tsv("../Results/Sex_PCW_12_20_FDR_0.1_DESeq/tables/MalevsFemale.complete.txt", col_types = cols(.default = col_character())) %>%
  select(Id, maxCooks) %>%
  mutate(ageBin='12-19')
for (ageBin in c('12', '13', '14', '15_17', '17_20') ) {
  DESeqCooks <- read_tsv(paste0("../Results/Sex_", ageBin, "_FDR_0.1_DESeq/tables/MalevsFemale.complete.txt"), col_types = cols(.default = col_character())) %>%
  select(Id, maxCooks) %>%
  mutate(ageBin=ifelse(ageBin=='15_17', '15-16', ifelse(ageBin=='17_20', '17-19', ageBin))) %>% 
