...
ANOS1_tr <- exonsBy(txdb, filter=GeneIdFilter('ENSG00000011201'))
p1 <- autoplot(ANOS1_tr, group.selfish = TRUE)
p2 <- autoplot(txdb, which = GeneIdFilter('ENSG00000011201'), stat="reduce")
gene_models <- tracks(transcripts = p1, gene = p2, heights = c(3,1))
pdf("Fig2A.pdf")
gene_models
dev.off()


ANOS1mean <- filter(fittedBias_tr, GeneId == 'ENSG00000011201') %>%
  bind_rows(filter(fittedBias, Id == 'ENSG00000011201')) %>%
...
ANOS1mean$facet<-factor(ANOS1mean$facet, levels=rev(levels(ANOS1mean$facet)))

  
ANOS1counts <- filter(counts_tr, GeneId == 'ENSG00000011201') %>%
  bind_rows(filter(counts, Id=='ENSG00000011201')) %>%
      ...
      left_join(dplyr::select(ANOS1mean, Id, qval) %>% group_by(Id) %>% dplyr::slice(1)) %>%
      mutate(facet=factor(paste0(Id, '\nFDR=', signif(qval, digits = 3))))
ANOS1counts$facet<-factor(ANOS1counts$facet, levels=rev(levels(ANOS1counts$facet)))

ggplot(ANOS1counts, aes(x=Sex, colour=Sex)) + 
    geom_jitter(aes(y=value), height = 0, width=.1, alpha=.75, size=1) + 
    geom_errorbar(aes(ymin=mean, ymax=mean), colour='black', size=1, width=.5, data=ANOS1mean) +
