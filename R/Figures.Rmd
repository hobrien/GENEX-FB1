...
fittedBias %>% filter(padj<.1 & ChrType == 'autosome') %>%
  group_by(gene_type) %>% 
  summarise(n=n(), proportion = n/nrow(filter(fittedBias %>% filter(padj<.1 & ChrType == 'autosome')))*100, DE='SexBiased') %>%
  bind_rows(fittedBias %>% filter(!is.na(padj) & ChrType == 'autosome') %>%
              group_by(gene_type) %>% 
              summarise(n=n(), proportion = n/nrow(filter(fittedBias %>% filter(!is.na(padj) & ChrType == 'autosome')))*100, DE='Background')) %>%
...
"\t", escape_double = FALSE, trim_ws = TRUE)
gene_sets <- fittedBias %>% filter(!is.na(padj)) %>%
...
filter(gene_sets, set=='Background') %>% 
  ...
  mutate(Background=1) %>% 
  right_join(filter(gene_sets, set=='SandersASD' & !is.na(ENSEMBL))) %>% 
  mutate(expressed=!is.na(Background)) %>%
  filter(!expressed)
  
filter(gene_sets, set=='SandersASD') %>% inner_join(DEall)
```
