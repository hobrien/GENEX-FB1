...
fittedBias <- fittedBias %>% filter(!is.na(gene_type)) %>%
  mutate(gene_type = ifelse(gene_type %in% c("antisense_RNA", "lincRNA", "protein_coding", "miRNA"),
                            gene_type, 
...
fittedBias %>% filter(padj<.1 & ChrType == 'autosome') %>%
  group_by(gene_type) %>% 
  summarise(n=n(), proportion = n/nrow(filter(fittedBias %>% filter(padj<.1 & ChrType == 'autosome'))), DE='SexBiased') %>%
  bind_rows(fittedBias %>% filter(!is.na(padj) & ChrType == 'autosome') %>%
              group_by(gene_type) %>% 
              summarise(n=n(), proportion = n/nrow(filter(fittedBias %>% filter(!is.na(padj) & ChrType == 'autosome'))), DE='Background')) %>%
...
#write_tsv(gene_types, "../Tables/gene_types.txt")
```
...
"\t", escape_double = FALSE, trim_ws = TRUE)
gene_sets <- fittedBias %>% mutate(set="Background", reference=NA) %>% 
...
group_by(gene_sets, set) %>% summarise(n())

as.data.frame(enricher(filter(DEall, (level=='both' | level=='gene-level') & ChrType=='autosomal')$Id, TERM2GENE=dplyr::select(gene_sets, set, ENSEMBL), pAdjustMethod='bonferroni', pvalueCutoff = .2, maxGSSize = 10000))

as.data.frame(enricher(filter(DEall, (level=='both') & ChrType=='autosomal')$Id, TERM2GENE=dplyr::select(gene_sets, set, ENSEMBL), pAdjustMethod='bonferroni', pvalueCutoff = .2, maxGSSize = 10000))

as.data.frame(enricher(filter(fittedBias, pvalue<.05 & ChrType=='autosome' & log2FoldDiff>0)$Id, TERM2GENE=dplyr::select(gene_sets, set, ENSEMBL), pAdjustMethod='bonferroni', pvalueCutoff = .2, maxGSSize = 10000))

as.data.frame(enricher(filter(fittedBias, pvalue<.05 & ChrType=='autosome' & log2FoldDiff<0)$Id, TERM2GENE=dplyr::select(gene_sets, set, ENSEMBL), pAdjustMethod='bonferroni', pvalueCutoff = .2, maxGSSize = 10000))


filter(gene_sets, set=='Background') %>% 
  dplyr::select(ENSEMBL) %>% 
  mutate(Background=1) %>% 
  right_join(filter(gene_sets, set=='XlinkedID' & !is.na(ENSEMBL))) %>% 
  mutate(expressed=!is.na(Background)) %>%
  group_by(expressed) %>% 
  summarise(n())

filter(gene_sets, set=='XlinkedID') %>% inner_join(DEall)
```
