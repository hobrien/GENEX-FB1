...
fittedBias <- read_delim("../Shiny/GENEX-FB1/Data/fitted.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  ...
  mutate(Sex = ifelse(log2FoldDiff < 0, 'F', 'M'), ChrType = ifelse(Chr %in% c('chrX','chrY'), Chr, 'autosome'), Chr = parse_factor(Chr, str_c('chr', c(seq(1,22), 'X', 'Y')))) 
...
sig_res <- fittedBiasAll %>% 
  bind_rows(fittedBias) %>% 
  filter(padj < 0.1) %>%
