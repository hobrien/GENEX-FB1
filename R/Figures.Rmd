...
left_join(XCI, fittedBias) %>%
  mutate(sig=ifelse(padj<.1, 1,0), Status=factor(Status, levels=c("E", "Mostly E", "VE",  "Mostly VE", "Mostly S", "S", "PAR", "No call", "Discordant" ))) %>%
  filter(SYMBOL != 'XIST') %>%
  ggplot(aes(x=Status, y=log2FoldDiff, colour=factor(sig))) +
