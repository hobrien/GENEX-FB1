...
XCI <- dplyr::select(NewFindings, GeneID, GeneName, Chr, Start, End, GTExStatus) %>%
  ...
  mutate(CombinedXCIstatus=ifelse(is.na(GTExStatus), CombinedXCIstatus, GTExStatus), Id=str_extract(GeneID, '^[^.]+')) %>%
  dplyr::select(-Chr)

XCI <- full_join(BallatonXCI, XCI, by=c('SYMBOL'='GeneName')) %>%
  left_join(dplyr::select(GTExXCI, GeneID, Region)) 
```
...
- No evidence of sex bias in new escape genes from GTEx
- Can't use the PAR/Non-PAR classification from GTEx table 13 because it only includes a subset of the genes in table 1 for some reason. Instead, I'm goin to borrow the PAR category from Ballaton.
```{r}
XCI_DE <- left_join(XCI, fittedBias, by='SYMBOL') %>% 
  mutate(sig=ifelse(padj<.1, ifelse(log2FoldDiff > 0, 1, 2), 0)) %>%
  filter(SYMBOL != 'XIST')

XCI_DE <- mutate(XCI_DE, CombinedXCIstatus = ifelse(Status=='PAR', 'PAR', CombinedXCIstatus))

filter(XCI_DE, ! is.na(GTExStatus)) %>% dplyr::select(SYMBOL, GTExStatus, log2FoldDiff, padj)

XCI_DE %>% filter(!is.na(CombinedXCIstatus)) %>% 
  mutate(CombinedXCIstatus=factor(CombinedXCIstatus, levels=c('inactive', 'variable', 'escape', 'PAR'))) %>%
  ggplot(aes(x=CombinedXCIstatus, y=log2FoldDiff, colour=factor(sig))) +
...
```{r}
XCI_DE %>%  dplyr::select(GeneID, SYMBOL, Status=CombinedXCIstatus, log2FoldDiff, pvalue, padj) %>% 
  filter(!is.na(padj) & !is.na(Status)) %>% 
  arrange(padj) %>% 
