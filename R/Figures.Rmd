...
```{r}
common <- DEall %>% group_by(gene_type) %>% 
  dplyr::summarise(n=n()) %>% 
  filter(n>10) %>% 
  semi_join(DEall, .) %>% 
  group_by(ChrType, gene_type, level) %>% 
  dplyr::summarise(n=n()) 

other <- DEall %>% group_by(gene_type) %>% 
  dplyr::summarise(n=n()) %>% 
  filter(n>10) %>% 
  anti_join(DEall, .) %>%
  group_by(ChrType, level) %>% 
  dplyr::summarise(n=n()) %>%
  mutate(gene_type='other')

rank <-common %>% group_by(gene_type) %>% dplyr::summarise(freq=sum(n)) %>% arrange(desc(freq))

c(rank$gene_type, 'other')
bind_rows(common, other) %>%
  spread(level, n) %>% 
  mutate(gene_type=factor(gene_type, levels=c(rank$gene_type, 'other'))) %>%
  arrange(ChrType, gene_type) %>% write_tsv("../Tables/gene_types.txt")
```
...
filter(gene_sets, set=='Background') %>% 
  ...
  mutate(Background=1) %>% 
  right_join(filter(gene_sets, set=='SFARI' & !is.na(ENSEMBL))) %>% 
