...
fittedBias <- read_delim("../Shiny/GENEX-FB1/Data/fitted.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  ...
  mutate(Sex = ifelse(log2FoldDiff < 0, 'F', 'M'), ChrType = ifelse(Chr %in% c('chrX','chrY'), Chr, 'autosome')) 
...
png(file="../Results/Fig1.png", bg="transparent", width=183, height=70, units='mm', res=300)
grid.arrange(Fig1a, Fig1b, ncol=2, widths=c(1,2))
dev.off()
pdf(file="../Results/Fig1.pdf", 7.2, 3)
grid.arrange(Fig1a, Fig1b, ncol=2, widths=c(1,2))
dev.off()
...
png(file="../Results/Fig2.png", bg="transparent", width=183, 
    height=70, units='mm', res=300)
grid.arrange(PlotFC(FR, 'A.'), PlotFC(AN, 'B.'), ncol=2)
dev.off()
pdf(file="../Results/Fig2.pdf", 7.2, 3)
grid.arrange(PlotFC(FR, 'A.'), PlotFC(AN, 'B.'), ncol=2)
dev.off()
...
png(file="../Results/Fig3.png", bg="transparent", width=183, height=140, units='mm', res=300)
grid.arrange(Fig3a, Fig3b, Fig3c, Fig3d, ncol=2)
dev.off()
pdf(file="../Results/Fig3.pdf", 7.2, 5.5)
grid.arrange(Fig3a, Fig3b, Fig3c, Fig3d, ncol=2)
dev.off()
...
pdf(file="../Results/FigS1.pdf", 7.2, 5.5)
grid.arrange(p1, p2, ncol=2)
dev.off()
png(file="../Results/FigS1.png", bg="transparent", width=183, height=140, units='mm', res=300)
grid.arrange(p1, p2, ncol=2)
dev.off()
grid.arrange(p1, p2, ncol=2)
...
png(file="../Results/FigS2.png", bg="transparent", width=183, 
...
grid.arrange(PlotFC(NPC, 'A. iPSC-derived neural progenitor cell genes'),
             ...
             ncol=1)
dev.off()
pdf(file="../Results/FigS2.pdf", 7.2, 9)
grid.arrange(PlotFC(NPC, 'A. iPSC-derived neural progenitor cell genes'),
             PlotFC(RG, 'B. Radial glia genes'),
             PlotFC(Neuronal, 'C. Fetal neuron genes'), 
             ncol=1)
...
write_tsv(Enrichment, "../Results/Enrichment14.txt")
Enrichment
```

All weeks (65 high-cofindence Autism genes)
```{r}

disease2gene <- filter(gene_sets, set %in% c('SandersASD', 'Background'))[, c("set", "gene_name")] 
...
write_tsv(Enrichment, "../Results/EnrichmentAll.txt")
Enrichment
```

Supplementary Table 1
```{r}
DEG12_19 <- fittedBiasAll %>% filter(padj<0.1) %>%
  mutate(`FoldDiff` = 2^log2FoldDiff, pvalue = as.numeric(format(pvalue, digits=2)), padj = as.numeric(format(padj, digits=2))) %>%
  arrange(Chr)
DEG12_19 <- DEG12_19 %>% 
  ...
  bind_rows(filter(DEG12_19, Chr %in% c('chrX', 'chrY'))) %>%
...
write.xlsx2(as.data.frame(DEG12_19), 
            file="../Results/SupplemtalTables.xlsx", 
            row.names=FALSE,
            sheetName='TblS1')
```
...
sig_res <- fittedBiasAll %>% bind_rows(fittedBias) %>% filter(padj < 0.1) %>%
  mutate(result = paste0(format(log2FoldDiff, digits=2), ' (', format(padj, digits=2), ')')) %>%
  dplyr::select(Id, SYMBOL, Chr, ageBin, result) %>%
  spread(ageBin, result) %>% 
  arrange(Chr)

sig_res <- sig_res %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(sig_res, Chr %in% c('chrX', 'chrY')))

sig_res <-sig_res %>% dplyr::select(`Ensembl ID` = Id, 
                `Gene ID` = SYMBOL,
                Chromosome = Chr,
                `12-19 PCW` = `12-19`,
                `12 PCW` = `12`,
...
DEG12 <- DEG12 %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(DEG12, Chr %in% c('chrX', 'chrY'))) %>%
  dplyr::select(`Ensembl ID` = Id, 
         `Gene ID` = SYMBOL,
         Chromosome = Chr,
         `Mean Count Male` = Male, 
...
DEG13 <- DEG13 %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(DEG13, Chr %in% c('chrX', 'chrY'))) %>%
  dplyr::select(`Ensembl ID` = Id, 
         `Gene ID` = SYMBOL,
         Chromosome = Chr,
         `Mean Count Male` = Male, 
...
DEG14 <- DEG14 %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(DEG14, Chr %in% c('chrX', 'chrY'))) %>%
  dplyr::select(`Ensembl ID` = Id, 
         `Gene ID` = SYMBOL,
         Chromosome = Chr,
         `Mean Count Male` = Male, 
...
DEG15_16 <- DEG15_16 %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(DEG15_16, Chr %in% c('chrX', 'chrY'))) %>%
  dplyr::select(`Ensembl ID` = Id, 
         `Gene ID` = SYMBOL,
         Chromosome = Chr,
         `Mean Count Male` = Male, 
...
DEG17_19 <- DEG17_19 %>%  filter(! Chr %in% c('chrX', 'chrY')) %>%
  mutate(Chr = sub("chr", "", Chr)) %>%
  arrange(as.numeric(Chr)) %>%
  mutate(Chr = paste0("chr",as.character(Chr))) %>%
  bind_rows(filter(DEG17_19, Chr %in% c('chrX', 'chrY'))) %>%
  dplyr::select(`Ensembl ID` = Id, 
         `Gene ID` = SYMBOL,
         Chromosome = Chr,
         `Mean Count Male` = Male, 
