...
counts <- read_delim("~/BTSync/FetalRNAseq/LabNotes/Results/BG12_19.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::select(Id, starts_with('norm'))

right_join(gene_info, counts) %>% 
  write_tsv("~/BTSync/FetalRNAseq/GENEX-FB1/Shiny/GENEX-FB1/Data/counts.txt")

fittedBias <- read_delim("~/BTSync/FetalRNAseq/LabNotes/Results/BG12_19.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
...
right_join(gene_info, fittedBias) %>% 
  write_tsv("~/BTSync/FetalRNAseq/GENEX-FB1/Shiny/GENEX-FB1/Data/fitted.txt")
