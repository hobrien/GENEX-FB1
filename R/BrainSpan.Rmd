...
plot_categories <-function(columns_metadata, hc, column) {
  ...
  ggplot(columns_metadata, aes(order, 1, fill = factor(trait))) +
     geom_tile() +
...
batch <- combined_metadata$source
modcombat = model.matrix(~1, data=select(combined_metadata, -structure, -donor))
combat_res = ComBat(dat=select(combined_qn, -Id), batch=batch, mod=modcombat, par.prior=TRUE, prior.plots=FALSE)
...
(p1<-ggplot(combat_pca_df, aes(x=PC1, y=PC2, colour=structure)) + 
      geom_point() +
      xlab(sprintf("PC1 (%0.1f%% explained var.)", pca_prop(combat_pca, 1))) +
      ylab(sprintf("PC2 (%0.1f%% explained var.)", pca_prop(combat_pca, 2))) +
      fte_theme() +
...
qn_ddata$labels <- qn_ddata$labels %>% mutate(Sample=colnames(combat_res)) %>%
   full_join(combined_metadata)
```
