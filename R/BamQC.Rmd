...
``` {r }
#bam_stat.py
RSeQCstats <- read_tsv("../Figures/read_numbers.txt", col_types = cols(sample = 'c'))
RSeQCstats <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCstats, by='sample')
knitr::kable(
...
RSeQCdistribution <- read_tsv("../Figures/read_distribution.txt", col_types = cols(sample = 'c'))
RSeQCdistribution <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCdistribution, by='sample')
RSeQCdistribution <- dplyr::select(RSeQCstats, sample, Unique) %>% 
  ...
  dplyr::select(sample, `Read Length`, tags_per_read, CDS, UTR, Intron, Intergenic)
knitr::kable(
  RSeQCdistribution
)
...
```{r }
RSeQCexpt <- read_tsv("../Figures/read_strand.txt", col_types = cols(sample = 'c'))
RSeQCexpt <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCexpt, by='sample')
...
RSeQCdistance <- read_tsv("../Figures/read_distance.txt", col_types = cols(sample = 'c'))
RSeQCdistance <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCdistance, by='sample')

ggplot(RSeQCdistance, aes(x=size, y=count, colour=`Read Length`, group=sample)) +
  geom_line() +
  ylab('reads') +
  xlab('inner distance') +
...
RSeQCsat <- read_tsv("../Figures/junction_sat.txt", col_types = cols(sample = 'c'))
RSeQCsat <- mutate(RSeQCsat, BrainBankID=str_extract(sample, "^[^-]+")) %>%
  full_join(dplyr::select(LibraryInfo, BrainBankID=Sample, `Read Length`), by='BrainBankID') %>% 
  full_join(select(RSeQCstats, BrainBankID=sample, ReadNumbers=Unique), by='BrainBankID') %>%
  dplyr::select(-BrainBankID)

ggplot(RSeQCsat, aes(y=junctions, x=ReadNumbers*percent_reads/100, group=sample, colour=`Read Length`)) +
  geom_point() +
  #xlim(c(0,500)) +
  #scale_y_log10() +
  scale_colour_brewer(type = "qual", palette = 6) +
