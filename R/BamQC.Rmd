...
RSeQCstats <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCstats)
knitr::kable(
...
RSeQCdistribution <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCdistribution)
dplyr::select(RSeQCstats, sample, Unique) %>% 
  full_join(RSeQCdistribution) %>%
...
RSeQCexpt <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCexpt)
...
RSeQCdistance <- dplyr::select(LibraryInfo, sample=Sample, `Read Length`) %>% full_join(RSeQCdistance)
...
RSeQCsat <- mutate(RSeQCsat, BrainBankID=str_extract(sample, "^[^-]+")) %>%
  full_join(dplyr::select(LibraryInfo, BrainBankID=Sample, `Read Length`)) %>% 
  full_join(select(RSeQCstats, BrainBankID=sample, ReadNumbers=Unique)) %>%
